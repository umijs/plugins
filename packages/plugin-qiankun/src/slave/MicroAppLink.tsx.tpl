...
import React, { forwardRef, FC, useImperativeHandle, useRef, MouseEvent, HTMLAttributes, useCallback } from 'react';
import noop from "{{{ lodashPath }}}/noop";
import find from "{{{ lodashPath }}}/find";
...
const urlFactory = (routes: Record<string, any>[]) => ({ appName, appRoute, isMaster }: MicroAppLinkProps) => {
  if (isMaster) {
    return appRoute;
  }
  ...
  return `${prefix}/${appRoute.startsWith('/') ? appRoute.slice(1) : appRoute}`;
};

export const MicroAppLink: FC<MicroAppLinkProps & HTMLAttributes<HTMLAnchorElement>> = forwardRef((props, ref) => {
  const {
    children,
    appName,
    appRoute,
  ...
  const linkUrl = historyType === 'hash' ? `#${createHerf(linkProps)}` : createHerf(linkProps);
  ...
  const handleClick = useCallback((e: MouseEvent<HTMLAnchorElement>) => {
    ...
    console.error(`[${COMPONENT_NAME}] not support "masterHistoryType = '${historyType}'"`)
    return;
